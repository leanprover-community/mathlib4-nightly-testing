...
-- Note: PR #13557 makes `mutual instance` blocks fail because the auto-generated
-- `_mutual` declaration's type (a `PSum.casesOn` returning `Decidable …`) is no longer
-- recognized as a type class at the new stricter transparency. Workaround: use `mutual def`
-- and provide the instances separately.
mutual
  def Equiv.decidableImpl : ∀ l₁ l₂ : Lists α, Decidable (l₁ ~ l₂)
    | ⟨false, l₁⟩, ⟨false, l₂⟩ =>
  ...
  def Subset.decidableImpl : ∀ l₁ l₂ : Lists' α true, Decidable (l₁ ⊆ l₂)
  ...
  termination_by x y => sizeOf x + sizeOf y
  def mem.decidableImpl : ∀ (a : Lists α) (l : Lists' α true), Decidable (a ∈ l)
    | a, Lists'.nil => isFalse <| by rintro ⟨_, ⟨⟩, _⟩
    | a, Lists'.cons' b l₂ => by
      haveI :=
        have : sizeOf (⟨_, b⟩ : Lists α) < 1 + 1 + sizeOf b + sizeOf l₂ := by simp [sizeof_pos]
        Equiv.decidableImpl a ⟨_, b⟩
      haveI :=
        have :
          SizeOf.sizeOf a + SizeOf.sizeOf l₂ <
            SizeOf.sizeOf a + SizeOf.sizeOf (Lists'.cons' b l₂) := by
          decreasing_tactic
        mem.decidableImpl a l₂
      refine decidable_of_iff' (a ~ ⟨_, b⟩ ∨ a ∈ l₂) ?_
      rw [← Lists'.mem_cons]; rfl
  termination_by x y => sizeOf x + sizeOf y
end

instance Equiv.decidable : ∀ l₁ l₂ : Lists α, Decidable (l₁ ~ l₂) := Equiv.decidableImpl
instance Subset.decidable : ∀ l₁ l₂ : Lists' α true, Decidable (l₁ ⊆ l₂) := Subset.decidableImpl
instance mem.decidable : ∀ (a : Lists α) (l : Lists' α true), Decidable (a ∈ l) := mem.decidableImpl
