...
theorem mul_toFun {m1 m2 : UnitMonomial} {basis : Basis} (h_basis : WellFormedBasis basis)
  ...
  | cons exp1 exps1 ih =>
    ...
    | cons basis_hd basis_tl =>
      ...
      -- Adapt to lean4#12824 (`List.eq_or_mem_of_mem_cons` grind pattern is now guarded, so
      -- `grind` no longer surfaces this cons-membership): prove it directly.
      have h2 : ∀ f ∈ basis_tl, 0 < f x := fun f hf => h_pos f (List.mem_cons_of_mem _ hf)
