...
-- Adaption note (lean4#13557): the field proofs below need
-- `set_option backward.defeqAttrib.useBackward true in` on each `dsimp`/
-- `simp` step that relies on `[backward_defeq]` simp lemmas registered with
-- `dsimp%` LHS patterns.
noncomputable def toSimplicialObjectHomotopy (H : Homotopy f g) :
  ...
  h_zero_comp_δ_zero n := by
    ...
    set_option backward.defeqAttrib.useBackward true in dsimp
    apply congr_arg
    ext k : 2
