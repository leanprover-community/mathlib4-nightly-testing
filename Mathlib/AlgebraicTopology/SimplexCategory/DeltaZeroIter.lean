...
lemma σ₀Iter_succ (i : ℕ) {n m : ℕ} (h : n + (i + 1) = m) :
  ...
  · rw [σ₀Iter_coe_eq_of_lt .., coe_σ]
    ...
    `Fin`-level antecedent `(σ₀Iter i _) k ≤ Fin.castSucc 0` from those `val` facts. This is
    the order analogue of the `Fin`-equality lifting that also broke in `Incenter`,
    `FormalGroup` and `Intervals`, chained through a second conditional rule. Note
