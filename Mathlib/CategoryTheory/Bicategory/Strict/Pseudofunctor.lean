...
#adaptation_note /-- lean4#13557: `useBackward true` is required for the `_app` and
...
differs subtly and `IsPrestack.overMapCompPresheafHomIso` (and possibly other downstream
callers) cannot supply `rfl` for the auxiliary equality hypotheses. -/
set_option backward.defeqAttrib.useBackward true in
@[to_app (attr := reassoc)]
...
-- see adaptation note above `mapComp'₀₁₃_inv_comp_mapComp'₀₂₃_hom` (lean4#13557).
set_option backward.defeqAttrib.useBackward true in
@[to_app (attr := reassoc)]
