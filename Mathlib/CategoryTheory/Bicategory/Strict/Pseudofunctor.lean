...
-- Adaptation note for lean4#13557: `useBackward true` is required for the `_app` and
-- `_app_assoc` forms generated by `to_app` to be properly normalized by `catAppSimp`,
-- which calls into simp with several `[backward_defeq]`-only lemmas (e.g.
-- `Cat.Hom.comp_toFunctor`, `Functor.comp_obj`). Without it, the generated lemma's type
-- differs subtly and `IsPrestack.overMapCompPresheafHomIso` (and possibly other
-- downstream callers) cannot supply `rfl` for the auxiliary equality hypotheses.
set_option backward.defeqAttrib.useBackward true in
@[to_app (attr := reassoc)]
...
-- See the adaptation note above `mapComp'₀₁₃_inv_comp_mapComp'₀₂₃_hom` (lean4#13557).
set_option backward.defeqAttrib.useBackward true in
@[to_app (attr := reassoc)]
lemma mapComp'₀₂₃_inv_comp_mapComp'₀₁₃_hom (hf : f₀₂ ≫ f₂₃ = f) :
    (F.mapComp' f₀₂ f₂₃ f).inv ≫ (F.mapComp' f₀₁ f₁₃ f).hom =
