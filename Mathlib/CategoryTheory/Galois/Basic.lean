...
lemma fiberEqualizerEquiv_symm_ι_apply {X Y : C} {f g : X ⟶ Y} (x : F.obj X)
  ...
  simp only [fiberEqualizerEquiv, comp_obj, Functor.comp_map, Iso.toEquiv_comp]
...
lemma fiberPullbackEquiv_symm_fst_apply {X A B : C} {f : A ⟶ X} {g : B ⟶ X}
  ...
  simp only [fiberPullbackEquiv, comp_obj, Functor.comp_map, Iso.toEquiv_comp,
...
lemma fiberBinaryProductEquiv_symm_fst_apply {X Y : C} (x : F.obj X) (y : F.obj Y) :
  ...
  simp only [fiberBinaryProductEquiv, comp_obj, Iso.toEquiv_comp]
