...
def module_set_option_pattern(option: str) -> re.Pattern:
    """Match module-level `set_option <option> false` (no trailing `in`)."""
    escaped = re.escape(option)
    return re.compile(rf"^set_option {escaped} false\s*$")
...
def scan_files(dag: DAG, option: str) -> dict[str, int]:
...
def main():
    ...
    removable_map = scan_files(full_dag, option)
